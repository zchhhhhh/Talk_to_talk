...
//    sendTextToOne("123",clientThread);
    qDebug() << "added";//调试用
 ...
 * 如果名字重复  会有消息
 */
void Server::on_client_usernameChanged(QString uname,ClientThread * client){
...
//! SLOTS
//注册
void Server::logon(QString name, QString password, QString sex, QString birth, QString major)
{
    qDebug() << name << password;
    if((!name.isEmpty())&&(!password.isEmpty()))
    {
        User user(name, password, sex, birth, major);
//        DataBase* pdb = DataBase::getDatabaseInstence();
        bool flag = database->findUsr(name);
        if(flag)//用户存在
        {
            foreach(ClientThread* client,clientThreadList){
                if(client->getUsername() == name){
                    Server::sendTextToOne("/value1:0\n",client);
                    return;
                }
            }
        }
        ...
        {
            bool ret = database->insertUsr(user);
            if (ret)//发送成功信息
            {
                foreach(ClientThread* client,clientThreadList){
                    if(client->getUsername() == name){
                        Server::sendTextToOne("/value1:1\n",client);
                        Server::on_client_usernameChanged(name,client);
                        return;
                    }
                }
            }
...
//    database = database->getDatabaseInstence();
    ...
    {
//    DataBase* pdb = DataBase::getDatabaseInstence();
        bool ret1 = database->findUsr(name);
        if(ret1)
        {
            bool ret2 = database->judge(name, password);
            if (ret2)
            {
                qDebug() << "login true" ;
                foreach(ClientThread* client,clientThreadList){
                    if(client->getUsername() == name){
                        Server::sendTextToOne("/value2:1\n",client);
                        Server::on_client_usernameChanged(name,client);
...
void Server::incomingConnection(qintptr socketDescriptor){
    ...
    ClientThread *cliThread = new ClientThread(socketDescriptor);
...
//    socket->moveToThread(cliThread);
    ...
    connect(cliThread, SIGNAL(logon(QString,QString,QString,QString,QString)),this,SLOT(logon(QString,QString,QString,QString,QString)));
    connect(cliThread, SIGNAL(login(QString,QString)),this,SLOT(login(QString,QString)));
